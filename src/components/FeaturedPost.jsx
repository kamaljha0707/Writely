            ...
            posts ? (
              <div className="bg-white text-[#373f45] rounded-lg w-full mb-6 flex flex-col-reverse sm:flex-row gap-6 justify-between items-center px-8 py-6 min-h-72">
                <div className="content h-full w-full sm:w-4/6 flex flex-col gap-4 items-start">
                  ...
                  <span className='font-semibold text-[#373f45c8] text-md capitalize flex items-center justify-between'>
                    <h1 className='flex justify-start gap-3 mt-2 items-center font-sans capitalize'>
                      <img src={profile} className='h-9 w-9' alt="" /> {posts?.username}
                    </h1>
                  </span>
                  <div className='mt-2'>
                ...
                <div className="post-img w-full sm:w-72 md:w-96 max-h-64 shadow-lg overflow-hidden rounded-lg">
                  <Link to={`/post/${posts?.$id}`}>
                    <img src={appwriteService.previewFile(posts?.featuredImage)} className='rounded-lg h-full w-full transition ease-in-out delay-150 duration-300 hover:scale-110' />
                  </Link>
                </div>
              </div>
            ) : null 
